...
class TestFetchByGroupid(object):
    ...
    def test_it_returns_only_documents_with_visible_annotations_for_user(
        self, svc, annotations, groups, users, db_session, matchers
    ):
        # This makes an "only me" annotation, associated with our target user
        annotations[1].shared = False
        annotations[1].userid = users["target_user"].userid

        # This makes an "only me" annotation, associated with a different user
        annotations[2].shared = False
        annotations[2].userid = users["other_user"].userid

        db_session.commit()

        docs = svc.fetch_by_groupid(
            groupid=groups["target_group"].pubid, userid=users["target_user"].userid
        )
...
@pytest.fixture
def users(factories):
    return {"target_user": factories.User(), "other_user": factories.User()}
...
@pytest.fixture
def annotations(factories, users, groups):
    return [
